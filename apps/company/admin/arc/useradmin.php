...
if (isset($_POST['save_user'])) {
    ...
    if (!empty($id)) {
        if (!empty($password)) {
            ...
            $sql = "UPDATE appuser SET username=?, password=?, personid=?, note=?, active=?, updatedat=NOW() WHERE id=?";
            $params = [$username, $hash, $personid, $note, $active, $id];
        } else {
            $sql = "UPDATE appuser SET username=?, personid=?, note=?, active=?, updatedat=NOW() WHERE id=?";
        ...
        $hash = password_hash($password, PASSWORD_DEFAULT);
        $sql = "INSERT INTO appuser (username, password, personid, note, active, createdat, updatedat) 
                VALUES (?, ?, ?, ?, ?, NOW(), NOW())";
