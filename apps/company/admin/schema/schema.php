...
function addColumn($pdo, $table, $column, $definition) {
    global $updateLog;
    $check = $pdo->query("SELECT column_name FROM information_schema.columns WHERE table_name = '$table' AND column_name = '$column' AND table_schema = 'public'");
...
function addIndex($pdo, $table, $indexName, $column) {
    ...
    $check = $pdo->query("SELECT indexname FROM pg_indexes WHERE tablename = '$table' AND indexname = '$indexName'");
