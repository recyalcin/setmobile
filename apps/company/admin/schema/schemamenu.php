...
try {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS menu (menuid SERIAL PRIMARY KEY);");

    // 1. Migration/Umbenennung alter Zeitstempel-Namen (falls vorhanden)
    $currentCols = $pdo->query("SELECT column_name FROM information_schema.columns WHERE table_name = 'menu'")->fetchAll(PDO::FETCH_COLUMN);
    ...
    $updatedCols = $pdo->query("SELECT column_name FROM information_schema.columns WHERE table_name = 'menu'")->fetchAll(PDO::FETCH_COLUMN);

    foreach ($columns as $col => $info) {
        if (!in_array($col, $updatedCols)) {
            $sql = "ALTER TABLE menu ADD $col {$info['type']}{$info['after']}";
            $pdo->exec($sql);
