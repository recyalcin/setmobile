...
if (isset($_POST['save'])) {
    ...
    if (!empty($_POST['id'])) {
        if (!empty($upass)) {
            ...
            $pdo->prepare("UPDATE appuser SET username=?, password=? WHERE id=?")->execute([$uname, $hash, $_POST['id']]);
        } else {
            $pdo->prepare("UPDATE appuser SET username=? WHERE id=?")->execute([$uname, $_POST['id']]);
        }
    } else {
        $hash = password_hash($upass, PASSWORD_DEFAULT);
        $pdo->prepare("INSERT INTO appuser (username, password) VALUES (?,?)")->execute([$uname, $hash]);
    }
    header("location: ./user?success=1"); exit;
}

if (!empty($id)) {
    $stmt = $pdo->prepare("SELECT username FROM appuser WHERE id=?");
    $stmt->execute([$id]);
    $username = $stmt->fetchColumn();
}
$list = $pdo->query("SELECT id, username FROM appuser")->fetchAll();
?>
