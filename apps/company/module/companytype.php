...
if (isset($_GET['delete'])) {
    ...
    $redirect = "/companytype&msg=deleted";
}
...
if (isset($_POST['save_companytype'])) {
    ...
    if (!empty($id)) {
        ...
        $redirect = "/companytype&msg=updated";
    } else {
        $sql = "INSERT INTO companytype (name, note, createdat) VALUES (?, ?, NOW())";
        $pdo->prepare($sql)->execute($params);
        $redirect = "/companytype&msg=created";
    }
}
...
<div class="card" style="margin-bottom: 20px; background: #f8fafc; border: 1px solid #cbd5e1;">
    <form method="get" action="/" style="display: flex; gap: 10px; align-items: flex-end; flex-wrap: wrap;">
        ...
        <a href="/companytype" class="btn reset-btn" style="height: 38px; display: inline-flex; align-items: center; background: #cbd5e1; color: #333; text-decoration: none; padding: 0 15px; border-radius: 4px; font-size: 14px; font-weight: 600;">Reset</a>
    </form>
</div>
...
$renderForm = function() use ($edit) { ?>
    <div class="card" style="margin-bottom: 25px; border-left: 5px solid #10b981;">
        ...
        <form method="post" action="/companytype" class="form-container">
            ...
            <div style="display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px; border-top: 1px solid #f1f5f9; padding-top: 15px;">
                <?php if($edit): ?>
                    <a href="/companytype&delete=<?= $edit['id'] ?>" class="btn-action delete-bg" onclick="return confirm('Typ wirklich löschen?')">🗑 Löschen</a>
                    <a href="/companytype" class="btn-action cancel-bg">Abbrechen</a>
                <?php endif; ?>
...
$renderList = function() use ($list, $totalPages, $page, $f_q) { ?>
    <div class="card">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align:right;">
                        <a href="/companytype&edit=<?= $ct['id'] ?>" class="action-link edit-link" style="font-size: 18px; text-decoration: none;">✎</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        ...
        <div class="pagination" style="margin-top: 25px; display: flex; justify-content: center; gap: 8px;">
            <?php $pUrl = "/companytype&q=".urlencode($f_q)."&p="; ?>
