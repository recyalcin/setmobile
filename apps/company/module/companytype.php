<?php
/**
 * module/companytype.php
 * Unternehmenstypen: Formular + Journal mit dynamischem Layout
 */
...
if (isset($_GET['delete'])) {
    $stmt = $pdo->prepare("DELETE FROM companytype WHERE id = ?");
    $stmt->execute([$_GET['delete']]);
    $redirect = "/?route=module/companytype&msg=deleted";
}

if (isset($_POST['save_companytype'])) {
    $id    = $_POST['id'] ?? null;
    $name  = $_POST['name'] ?? '';
    $note  = $_POST['note'] ?? '';
    ...
    if (!empty($id)) {
        $sql = "UPDATE companytype SET name=?, note=?, updatedat=NOW() WHERE id=?";
        $params[] = $id;
        $pdo->prepare($sql)->execute($params);
        $redirect = "/?route=module/companytype&msg=updated";
    } else {
        $sql = "INSERT INTO companytype (name, note, createdat) VALUES (?, ?, NOW())";
        $pdo->prepare($sql)->execute($params);
        $redirect = "/?route=module/companytype&msg=created";
    }
}
...
$p_sql = [];
if ($f_q) {
    $where[] = "(ct.name LIKE ? OR ct.note LIKE ?)";
    $p_sql[] = "%$f_q%"; $p_sql[] = "%$f_q%";
}
$whereSql = implode(" AND ", $where);

$totalCount = $pdo->prepare("SELECT COUNT(*) FROM companytype ct WHERE $whereSql");
...
if (isset($_GET['edit']) && $_GET['edit'] !== 'new') {
    $stmt = $pdo->prepare("SELECT * FROM companytype WHERE id = ?");
    $stmt->execute([$_GET['edit']]);
    $edit = $stmt->fetch();
}

$listStmt = $pdo->prepare("SELECT ct.* FROM companytype ct WHERE $whereSql ORDER BY ct.name ASC LIMIT $limit OFFSET $offset");
...
<div class="card" style="margin-bottom: 20px; background: #f8fafc; border: 1px solid #cbd5e1;">
    <form method="get" action="/" style="display: flex; gap: 10px; align-items: flex-end; flex-wrap: wrap;">
        <input type="hidden" name="route" value="module/companytype">
        ...
        <a href="/?route=module/companytype" class="btn reset-btn" style="height: 38px; display: inline-flex; align-items: center; background: #cbd5e1; color: #333; text-decoration: none; padding: 0 15px; border-radius: 4px; font-size: 14px; font-weight: 600;">Reset</a>
    </form>
</div>
...
$renderForm = function() use ($edit) { ?>
    <div class="card" style="margin-bottom: 25px; border-left: 5px solid #10b981;">
        <h3 style="margin-bottom: 15px;">🏢 <?= $edit ? 'Typ bearbeiten' : 'Neuen Typ anlegen' ?></h3>
        <form method="post" action="/?route=module/companytype" class="form-container">
            ...
            <div style="display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px; border-top: 1px solid #f1f5f9; padding-top: 15px;">
                <?php if($edit): ?>
                    <a href="/?route=module/companytype&delete=<?= $edit['id'] ?>" class="btn-action delete-bg" onclick="return confirm('Typ wirklich löschen?')">🗑 Löschen</a>
                    <a href="/?route=module/companytype" class="btn-action cancel-bg">Abbrechen</a>
                <?php endif; ?>
                <button type="submit" name="save_companytype" class="btn save" style="padding: 10px 40px; font-weight: bold;">Speichern</button>
            </div>
        </form>
    </div>
<?php };
...
$renderList = function() use ($list, $totalPages, $page, $f_q) { ?>
    <div class="card">
        <table class="data-table">
            ...
            <tbody>
                <?php foreach ($list as $ct): ?>
                <tr>
                    <td><strong><?= htmlspecialchars($ct['name']) ?></strong></td>
                    <td style="font-size:12px; color:#64748b;"><?= htmlspecialchars($ct['note'] ?: '-') ?></td>
                    <td style="text-align:right;">
                        <a href="/?route=module/companytype&edit=<?= $ct['id'] ?>" class="action-link edit-link" style="font-size: 18px; text-decoration: none;">✎</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        ...
        <div class="pagination" style="margin-top: 25px; display: flex; justify-content: center; gap: 8px;">
            <?php $pUrl = "/?route=module/companytype&q=".urlencode($f_q)."&p="; ?>
