<?php
/**
 * module/employeetype.php
 * Verwaltung der Mitarbeiter-Typen (z.B. Vollzeit, Teilzeit, Praktikant)
 */
...
if (isset($_GET['msg'])) {
    $m = $_GET['msg'];
    if ($m === 'created') $message = '<div class="alert success">✅ Typ erfolgreich erstellt.</div>';
...
$edit = null;
if (isset($_GET['edit'])) {
    $stmt = $pdo->prepare("SELECT * FROM employeetype WHERE id = ?");
    $stmt->execute([$_GET['edit']]);
    $edit = $stmt->fetch();
}

// --- 5. LISTE LADEN ---
$list = $pdo->query("SELECT * FROM employeetype ORDER BY name ASC")->fetchAll();
?>
...
<div class="card" style="margin-bottom: 30px;">
    ...
    <form method="post" action="/employeetype" class="form-container">
        ...
        <div class="form-row">
            ...
            <input type="text" name="name" value="<?= htmlspecialchars($edit['name'] ?? '') ?>" placeholder="z.B. Festangestellt" required autofocus>
        </div>

        <div class="form-row" style="align-items: flex-start;">
            <label style="margin-top:10px;">Notiz</label>
            <textarea name="note" rows="3" placeholder="Zusätzliche Infos..."><?= htmlspecialchars($edit['note'] ?? '') ?></textarea>
        </div>
