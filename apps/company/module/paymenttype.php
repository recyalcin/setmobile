...
if (isset($_POST['save_paymenttype']) || isset($_POST['duplicate_paymenttype'])) {
    ...
    if (!empty($id)) {
        ...
        $redirect = "/paymenttype&msg=updated";
    } else {
        // INSERT
        $sql = "INSERT INTO paymenttype (name, note, createdat) VALUES (?, ?, NOW())";
        $pdo->prepare($sql)->execute($params);
        $redirect = "/paymenttype&msg=created";
    }
}
...
if (isset($_GET['delete'])) {
    ...
    $redirect = "/paymenttype&msg=deleted";
}
...
<div class="card" style="margin-bottom: 25px; border-left: 5px solid #06b6d4;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
        ...
        <a href="/paymenttype&edit=new" class="btn-action neu-bg" style="text-decoration:none;">+ Neue Zahlungsart</a>
    </div>

    <form method="post" action="/paymenttype" class="form-container">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 20px; border-top: 1px solid #f1f5f9; padding-top: 15px;">
            <div>
                <?php if($edit): ?>
                    <a href="/paymenttype&delete=<?= $edit['id'] ?>" class="btn-action delete-bg" onclick="return confirm('Zahlungsart wirklich löschen?')">🗑 Löschen</a>
                <?php endif; ?>
            </div>
            <div style="display: flex; gap: 10px;">
                <?php if($edit): ?>
                    <a href="/paymenttype" class="btn-action cancel-bg" style="text-decoration:none;">Abbrechen</a>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align:right;">
                    <a href="/paymenttype&edit=<?= $item['id'] ?>" class="edit-link" style="color:#06b6d4;">✎</a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
