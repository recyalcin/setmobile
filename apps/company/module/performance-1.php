...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'save') {
        ...
        $vals = [$post_driverid, $post_week];
        $updateParts = [];
        foreach($fields as $f) {
            $v = ($f == 'earningsperformancebolt') ? $val_eb : (($f == 'earningsperformanceuber') ? $val_eu : ($_POST[$f] ?? 0));
            $vals[] = $v;
            $updateParts[] = "\"$f\" = EXCLUDED.\"$f\"";
        }

        $sql = "INSERT INTO performance (driverid, week, " . implode(", ", array_map(fn($f) => "\"$f\"", $fields)) . ")
                VALUES (?, ?, " . str_repeat('?,', count($fields)-1) . "?)
                ON CONFLICT (driverid, week) DO UPDATE SET " . implode(", ", $updateParts);
...
$whereClauses = [];
$params = [];
if ($kisiid) { $whereClauses[] = "p.driverid = ?"; $params[] = $kisiid; }
...
$sql_list = "
    SELECT p.*, per.firstname, per.lastname,
    ...
    FROM performance p
    JOIN driver dr ON p.driverid = dr.id
    JOIN person per ON dr.personid = per.id
    LEFT JOIN (
        SELECT driverid,
    ...
    $whereSql
    ORDER BY stats.avg_perf_total DESC, p.driverid, p.week DESC
";
...
$kisilist = $pdo->query("SELECT d.id, per.firstname, per.lastname FROM driver d JOIN person per ON d.personid = per.id ORDER BY per.firstname ASC")->fetchAll();
?>
                ...
                <select name="f_kisiid" class="form-control">
                    ...
                    <?php foreach($kisilist as $k): ?>
                        <option value="<?= $k['id'] ?>" <?= $kisiid == $k['id'] ? 'selected' : '' ?>>
                            <?= htmlspecialchars($k['firstname'].' '.$k['lastname']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
