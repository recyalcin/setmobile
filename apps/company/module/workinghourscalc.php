...
if (isset($_POST['calculate_workinghours'])) {
    try {
        ...
        $pdo->prepare("UPDATE workinghours SET workstartat=NULL, workendat=NULL, hourstotal=0, startkm=0, endkm=0, breakduration=0 WHERE date LIKE ?")->execute([$queryDate]);
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
            ...
            $pdo->prepare("UPDATE workinghours SET startkm=?, hourstotal=? WHERE id=?")->execute([$h04, ($tsE - $tsS) / 3600, $r['id']]);
        }
        ...
        foreach ($all as $x) {
            ...
            if (($x['hourstotal'] * $mult) >= 4) {
                ...
                $pdo->prepare("UPDATE workinghours SET hourstotal = hourstotal * ?, startkm = startkm * ? WHERE id = ?")->execute([$mult, $mult, $x['id']]);
            }
        }
        ...
        // --- 7. RUNDEN (0,25) & PAUSE ---
        $stmt = $pdo->prepare("SELECT id, workstartat, workendat, hourstotal, startkm FROM workinghours WHERE date LIKE ? AND workstartat IS NOT NULL");
        $stmt->execute([$queryDate]);
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
            ...
            $h206 = round(max(0, $ov206 - (float)$r['startkm']) / 0.25) * 0.25;

            $pdo->prepare("UPDATE workinghours SET hourstotal=?, startkm=round(startkm/0.25)*0.25, endkm=?, breakduration=? WHERE id=?")
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
            ...
            if ((float)$r['hourstotal'] < 0 || (float)$r['startkm'] < 0 || (float)$r['breakduration'] < 0 || $sum != round($pres, 2)) {
                ...
                $pdo->prepare("UPDATE workinghours SET workstartat=?, workendat=?, hourstotal=?, startkm=?, endkm=?, breakduration=? WHERE id=?")
