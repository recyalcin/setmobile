...
function updateDayRecord($db, $row) {
    $stmt = $db->prepare("
        UPDATE workinghours SET
            ...
            lasttripat = :lasttripat, breakduration = :breakduration, startkm = :startkm,
            endkm = :endkm, hourstotal = :hourstotal
        WHERE id = :id
    ");
    $stmt->execute([
        ':workstartat' => $row['workstartat'], ':workendat' => $row['workendat'],
        ':firsttripat'  => $row['firsttripat'],  ':lasttripat'  => $row['lasttripat'],
        ':breakduration'=> $row['breakduration'], ':startkm'   => $row['startkm'] ?? 0,
        ':endkm'    => $row['endkm'] ?? 0,    ':hourstotal'  => $row['hourstotal'],
        ':id'           => $row['id']
    ]);
}
...
foreach ($allDrivers as $driverId) {
    ...
    for ($d = 1; $d <= $daysInMonth; $d++) {
        ...
        $db->prepare("INSERT INTO workinghours (employeeid, date) VALUES (?, ?)
                      ON CONFLICT (employeeid, date) DO UPDATE SET workstartat=NULL, workendat=NULL, firsttripat=NULL, lasttripat=NULL,
                      breakduration=0, startkm=0, endkm=0, hourstotal=0")->execute([$driverId, $date]);
    }
