...
/**
 * module/country.php
 * Länder: Formular + Journal mit dynamischem Layout
 */
...
$p_sql = [];
if ($f_q) {
    $where[] = "(c.name LIKE ? OR c.code LIKE ? OR c.note LIKE ?)";
    $p_sql[] = "%$f_q%"; $p_sql[] = "%$f_q%"; $p_sql[] = "%$f_q%";
}
$whereSql = implode(" AND ", $where);

$totalCount = $pdo->prepare("SELECT COUNT(*) FROM country c WHERE $whereSql");
...
$listStmt = $pdo->prepare("SELECT c.* FROM country c
...
<div class="card" style="margin-bottom: 20px; background: #f8fafc; border: 1px solid #cbd5e1;">
    <form method="get" action="/" style="display: flex; gap: 10px; align-items: flex-end; flex-wrap: wrap;">
        <input type="hidden" name="route" value="module/country">
        <div style="flex: 1; min-width: 250px;">
            <label class="filter-label">🔍 Suche (Name, Code, Notiz)</label>
    ...
    <div class="card" style="margin-bottom: 25px; border-left: 5px solid #10b981;">
        ...
        <form method="post" action="/?route=module/country" class="form-container">
            ...
            <div class="form-row">
                ...
                <input type="text" name="name" value="<?= htmlspecialchars($edit['name'] ?? '') ?>" placeholder="z.B. Deutschland" required>
            </div>

            <div class="form-row">
                <label>ISO-Code</label>
                <input type="text" name="code" value="<?= htmlspecialchars($edit['code'] ?? '') ?>" placeholder="z.B. DE, AT, CH" style="width: 100px; flex-grow: 0;">
            </div>
