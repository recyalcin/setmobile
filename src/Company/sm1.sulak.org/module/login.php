...
if (!isset($_SESSION['userid']) && isset($_COOKIE['rememberme'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM user WHERE remembertoken = ? LIMIT 1");
    $stmt->execute([$token]);
    $user = $stmt->fetch();
...
if (isset($_POST['dologin'])) {
    ...
    if ($user && password_verify($passIn, $user['password'])) {
        ...
        if ($keepLoggedIn) {
            ...
            $stmt = $pdo->prepare("UPDATE user SET remembertoken = ? WHERE id = ?");
            $stmt->execute([$newToken, $user['id']]);

            // Cookie für 30 Tage setzen (httponly für Sicherheit)
