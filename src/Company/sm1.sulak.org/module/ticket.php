    ...
    $sql = "SELECT t.id FROM ticket t
            ...
            ORDER BY 
                (CASE 
                    ...
                    WHEN t.dueat IS NOT NULL AND t.dueat <= DATE_ADD(NOW(), INTERVAL 24 HOUR) THEN 2
                    WHEN t.scheduledat IS NOT NULL AND t.scheduledat < NOW() THEN 3
                    WHEN t.scheduledat IS NOT NULL AND t.scheduledat <= DATE_ADD(NOW(), INTERVAL 24 HOUR) THEN 4
                    ELSE 5
                END) ASC,
                t.resolvedat DESC,
                t.dueat ASC,
