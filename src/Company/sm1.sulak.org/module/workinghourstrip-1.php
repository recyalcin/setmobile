...
if (isset($_POST['run_pipeline'])) {
    try {
        ...
        // 2. Full Month Init (Nur 0004 und 2006 Spalten)
        $employees = $pdo->query("SELECT id FROM employee")->fetchAll(PDO::FETCH_COLUMN);
        $initStmt = $pdo->prepare("INSERT INTO workinghours (employeeid, date, firsttripat, lasttripat, workstartat, workendat, hourstotal, hours0004, hours2006, breakduration) VALUES (?, ?, NULL, NULL, NULL, NULL, 0, 0, 0, 0) ON DUPLICATE KEY UPDATE firsttripat=NULL, lasttripat=NULL, workstartat=NULL, workendat=NULL, hourstotal=0, hours0004=0, hours2006=0, breakduration=0");
        ...
        for ($i = 0; $i < count($rows); $i++) {
            ...
            if ($streak == 6) { 
                ...
                $pdo->prepare("UPDATE workinghours SET workstartat=NULL, workendat=NULL, hourstotal=0, hours0004=0, hours2006=0 WHERE id=?")->execute([$rows[$i]['id']]);
                $streak = -1;
            }
        }
        ...
        for ($i = 1; $i < count($rowsRest); $i++) {
            if ($rowsRest[$i]['employeeid'] == $rowsRest[$i-1]['employeeid']) {
                ...
                if ($gap < 39600) {
                    ...
                    $pdo->prepare("UPDATE workinghours SET workendat=?, hourstotal=(UNIX_TIMESTAMP(workendat)-UNIX_TIMESTAMP(workstartat))/3600 WHERE id=?")->execute([date('Y-m-d H:i:s', $newE), $rowsRest[$i-1]['id']]);
                    $pdo->prepare("UPDATE workinghours SET workstartat=?, hourstotal=(UNIX_TIMESTAMP(workendat)-UNIX_TIMESTAMP(workstartat))/3600 WHERE id=?")->execute([date('Y-m-d H:i:s', $newS), $rowsRest[$i]['id']]);
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
            ...
            $pdo->prepare("UPDATE workinghours SET hours0004=?, hours2006=? WHERE id=?")->execute([$raw04, $raw206, $r['id']]);
        }
        ...
        foreach ($rowsFinal as $r) {
            ...
            $new04 = min($r['hours0004'], $r['hours0004'] * $mult);
            $new206 = min($r['hours2006'], $r['hours2006'] * $mult);

            $pdo->prepare("UPDATE workinghours SET hourstotal=?, hours0004=?, hours2006=?, breakduration=? WHERE id=?")
                ->execute([$newT, $new04, $new206, $pres - $newT, $r['id']]);
        }

        // 7. Kompensation < 4h
        $pdo->query("UPDATE workinghours SET hourstotal = hourstotal + (breakduration / 2), breakduration = breakduration - (breakduration / 2) WHERE date LIKE '$queryDate' AND workstartat IS NOT NULL AND hourstotal < 4 AND breakduration > 0");

        // 8. Max 10h Regel
        $pdo->query("UPDATE workinghours SET breakduration = breakduration + (hourstotal - 10), hourstotal = 10 WHERE date LIKE '$queryDate' AND workstartat IS NOT NULL AND hourstotal > 10");
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
            ...
            $pdo->prepare("UPDATE workinghours SET hourstotal=?, hours0004=round(hours0004/0.25)*0.25, hours2006=round(hours2006/0.25)*0.25, breakduration=? WHERE id=?")
