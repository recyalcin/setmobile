    ...
    try {
        ...
        $save = function($row) use ($db) {
            $stmt = $db->prepare("UPDATE workinghours SET 
                workstartat=?, workendat=?, breakduration=?, hours0004=?, hours2006=?, 
                hourstotal=?, firsttripat=?, lasttripat=?, recordedat=NOW() 
                WHERE id=?");
            $stmt->execute([
                $row['workstartat'], $row['workendat'], $row['breakduration'], 
                $row['hours0004'], $row['hours2006'], $row['hourstotal'], 
        ...
        foreach ($employees as $emp) {
            ...
            for ($d = 1; $d <= $daysInMonth; $d++) {
                ...
                $db->prepare("INSERT INTO workinghours (employeeid, date, hourstotal) VALUES (?, ?, 0) 
                    ON DUPLICATE KEY UPDATE workstartat=NULL, workendat=NULL, breakduration=0, hours0004=0, hours2006=0, hourstotal=0, firsttripat=NULL, lasttripat=NULL, recordedat=NULL")
                    ->execute([$empId, $date]);
            }
                ...
                foreach ($data as $idx => $row) {
                    if ($row['workstartat']) {
                        ...
                        if ($streak == 7) {
                            ...
                            $db->prepare("UPDATE workinghours SET workstartat=NULL, workendat=NULL, hourstotal=0, breakduration=0, hours0004=0, hours2006=0 WHERE id=?")->execute([$targetId]);
                            break; 
                        }
            ...
            foreach ($data as $row) {
                ...
                $row['hours0004'] = min(4, $h0004);
                $row['hours2024'] = $h2024; // Hilfsvariable
                $row['hours2006'] = min(6, $h2024 + $h0406);
            ...
            if (count($workDays) > 0) {
                ...
                foreach ($data as $row) {
                    if ($row['workstartat']) {
                        ...
                        $row['hours0004'] *= $multiplier;
                        $row['hours2006'] *= $multiplier;
                    }
                    $save($row);
                }
            }
            ...
            foreach ($data as $row) {
                ...
                if ($row['hours0004'] > $max04) $row['hours0004'] = $max04;
                if ($row['hours2006'] > $max2006) $row['hours2006'] = $max2006;
            ...
            foreach ($data as $row) {
                ...
                $row['hours0004'] = round($row['hours0004'] * 4) / 4;
                $row['hours2006'] = round($row['hours2006'] * 4) / 4;
                $save($row);
            }
