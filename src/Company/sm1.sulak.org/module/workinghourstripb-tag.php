...
if (isset($_POST['run_pipeline'])) {
    try {
        ...
        foreach ($employees as $emp) {
            for ($d = 1; $d <= $daysInMonth; $d++) {
                ...
                $db->prepare("
                    ...
                    breakduration=0, hours0004=0, hours2006=0, hourstotal=0, recordedat=NULL
        ...
        $db->prepare("UPDATE workinghours SET 
            workstartat = FROM_UNIXTIME(ROUND((UNIX_TIMESTAMP(firsttripat) - 900) / 1800) * 1800), 
            workendat = FROM_UNIXTIME(ROUND((UNIX_TIMESTAMP(lasttripat) + 900) / 1800) * 1800) 
                ...
                foreach ($days as $idx => $d) {
                    if ($d['workstartat']) {
                        ...
                        if (count($streak) == 7) {
                            ...
                            $db->prepare("UPDATE workinghours SET workstartat=NULL, workendat=NULL, breakduration=0, hours0004=0, hours2006=0, hourstotal=0, recordedat=NULL WHERE id = ?")->execute([$targetId]);
                            break; 
                        }
        ...
        foreach ($employees as $emp) {
            ...
            foreach ($workDays as $wd) {
                ...
                $curHT = ($e - $s) / 3600;
                $totalHT_Pre += $curHT;
                $db->prepare("UPDATE workinghours SET hours0004=?, hours2006=?, hourstotal=? WHERE id=?")
            ...
            foreach ($load2->fetchAll(PDO::FETCH_ASSOC) as $wd2) {
                ...
                $newH0004 = $wd2['hours0004'] * $multiplier;
                $newH2006 = $wd2['hours2006'] * $multiplier;
                ...
                $db->prepare("UPDATE workinghours SET hourstotal=?, hours0004=?, hours2006=? WHERE id=?")
        ...
        foreach ($final->fetchAll(PDO::FETCH_ASSOC) as $f) {
            ...
            $db->prepare("UPDATE workinghours SET 
                ...
                hours0004 = ROUND(hours0004 * 4) / 4,
                hours2006 = ROUND(hours2006 * 4) / 4,
                workstartat = FROM_UNIXTIME(ROUND(UNIX_TIMESTAMP(workstartat) / 900) * 900),
                workendat = FROM_UNIXTIME(ROUND(UNIX_TIMESTAMP(workendat) / 900) * 900)
